...
let find_most_recent_bin (graph_fragment : G.t) : G.t option =
  let result =
    DataFlowEdges.graph_already_serialized ~comp_unit:graph_fragment.comp_unit ~suffix:"" ~finished:true
  in
  match result with
  | None ->
      None
  | Some file ->
      print_endline @@ F.asprintf "deserializing %s." file ;
      Deserializer.deserialize_graph file


let build_graph (graph_fragment : G.t) : G.t =
  match find_most_recent_bin graph_fragment with
  | None ->
...
let one_pass (graph_fragment : G.t) : unit =
  ...
  ignore @@ loop finished_graph NodeWiseFeatures.NodeWiseFeatureMap.empty
