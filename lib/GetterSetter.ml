...
let is_predicate (method_ : Method.t) =
  let starts_with_has_or_is =
    ...
    and starts_with_is = String.is_prefix ~prefix:"is" (Method.get_method_name method_) in
...
let is_user_defined_equals (method_ : Method.t) =
  String.equal (Method.get_method_name method_) "equals"
  && String.equal (Method.get_return_type method_) "boolean"
