      ...
      if dry_run then (graph, [])
        ...
        if
          List.exists
            ~f:(fun leaf ->
              TaintLabel.equal
                (ProbQuadruple.determine_label (Vertex.get_dist leaf))
                TaintLabel.Sink )
            trunk_leaves
        then
          List.fold
...
module AskingRules = struct
  ...
  let ask_indeterminate : rule =
   ...
   fun (snapshot : G.t) (received_responses : Response.t list)
    ...
    let random_indeterminate_vertex =
      Utils.random_select_elem
        (List.filter
           ~f:(fun vertex ->
             ProbQuadruple.is_indeterminate (Vertex.get_dist vertex)
  ...
  let ask_from_ns_cluster_if_it_contains_internal_src_or_sink : rule =
   fun (snapshot : G.t) (received_responses : Response.t list)
    ...
    let random_vertex_in_picked_cluster =
      Utils.random_select_elem
        (List.filter
           ~f:(fun vertex ->
             ProbQuadruple.is_indeterminate (Vertex.get_dist vertex)
