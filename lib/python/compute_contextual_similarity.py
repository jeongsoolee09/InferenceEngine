...
def read_trunks(json_filename):
    with open(json_filename, "r+") as jsonfile:
        ...
        trunks = list(
            map(
                lambda tup: (tup[0], list(map(lambda string: eval(string), tup[1]))),
                list(trunks.items()),
            )
        )
...
def read_redefines():
    with open("Redefines.txt", "r+") as redefines:
        lines_read = list(map(lambda method: method.rstrip(), redefines.readlines()))
        acc = dict()
...
class ContextualFeature:
    ...
    def same_callee_in_trunk_count(row):
        ...
        trunk1_methods = list(
            map(lambda tup: get_only_classname_and_method(tup[0]), row.trunk1)
        )
        trunk2_methods = list(
            map(lambda tup: get_only_classname_and_method(tup[0]), row.trunk2)
        )
        acc = 0
    ...
    def both_starts_and_ends_with_apis(row):
        ...
        trunk1_head_is_api = any(
            list(map(lambda line: trunk1_head_classname_and_method in line, apis))
        )
        trunk1_end_is_api = any(
            list(map(lambda line: trunk1_end_classname_and_method in line, apis))
        )
        trunk2_head_is_api = any(
            list(map(lambda line: trunk2_head_classname_and_method in line, apis))
        )
        trunk2_end_is_api = any(
            list(map(lambda line: trunk2_end_classname_and_method in line, apis))
        )

        if (
            trunk1_head_is_api
            and trunk1_end_is_api
            and trunk2_head_is_api
            and trunk2_end_is_api
        ):
            return 10
        else:
            return 0
    ...
    def both_starts_and_ends_with_resp_same_method(row):
        ...
        trunk1_head_matches_trunk1_end = (
            trunk1_head_classname_and_method == trunk1_end_classname_and_method
        )
        trunk2_head_matches_trunk2_end = (
            trunk2_head_classname_and_method == trunk2_end_classname_and_method
        )
    ...
    def both_has_methods_with_same_annot(row):
        ...
        trunk1_annotations = list(
            filter(lambda trunk1_method: get_annots(trunk1_method), trunk1_methods)
        )

        trunk2_annotations = list(
            filter(lambda trunk2_method: get_annots(trunk2_method), trunk2_methods)
        )

        # now, get only the annotation names
        # by omitting annotation parameters.

        trunk1_annotation_names = list(
            map(lambda str: str.split("(")[0], trunk1_annotations)
        )

        trunk2_annotation_names = list(
            map(lambda str: str.split("(")[0], trunk2_annotations)
        )
...
def get_trunk_similarity(row):
    return reduce(
        ...
        [
            ...
            ContextualFeature.both_starts_and_ends_with_resp_same_method,
        ],
        0,
    )
...
def leave_only_most_similar_pairs(carpro):
    ...
    for index in lhs_unique_values:
        ...
        rows_with_max_similarity_with_lhs = rows_with_this_index_as_lhs[
            rows_with_this_index_as_lhs.cs_score
...
def find_methods_to_connect(carpro_row):
    ...
    if any(list(map(lambda method: get_annots(method), trunk1))) and any(
        list(map(lambda method: get_annots(method), trunk2))
    ):

        trunk1_first_annot_method = list(
            filter(lambda method: get_annots(method), trunk1)
        )[0]
        trunk1_last_annot_method = list(
            filter(lambda method: get_annots(method), trunk1)
        )[-1]
        trunk2_first_annot_method = list(
            filter(lambda method: get_annots(method), trunk2)
        )[0]
        trunk2_last_annot_method = list(
            filter(lambda method: get_annots(method), trunk2)
        )[-1]
        root_pair_list = [
            (trunk1_first_annot_method, trunk2_first_annot_method),
            (trunk1_last_annot_method, trunk2_last_annot_method),
        ]
    elif not (trunk1_root == trunk2_root) and not (
        is_initializer(trunk1_root) or is_initializer(trunk2_root)
    ):
        root_pair_list = [(trunk1_root, trunk2_root), (trunk2_root, trunk1_root)]
    else:
        root_pair_list = []
        # computing leaf_pair_list
    trunk1_leaf = trunk1[-1]
    trunk2_leaf = trunk2[-1]
    if not (trunk1_leaf == trunk2_leaf) and not (
        is_initializer(trunk1_leaf) or is_initializer(trunk2_leaf)
    ):
        leaf_pair_list = [(trunk1_leaf, trunk2_leaf), (trunk2_leaf, trunk1_leaf)]
    else:
        leaf_pair_list = []
    redefines_carpro = []
    for method1 in trunk1:
        for method2 in trunk2:
            if is_redefine(method1) and is_redefine(method2) and (method1 != method2):
