...
source_comp_unit = None
target_comp_unit = None

def repl():
    os.chdir("../..")
    global source_comp_unit
    global target_comp_unit
...
def main():
    ...
    # apis
    apis_carpro = make_carpro_of_dataframe(source_apis, target_apis)
    api_nodewise_sim_column = apis_carpro.apply(run_all_pairwise_feature, axis=1)
    apis_carpro["ns_score"] = api_nodewise_sim_column
    apis_filtered_above_threshold = apis_carpro[apis_carpro.ns_score > ns_threshold]
    api_filtered = leave_only_most_similar_pairs(
        no_reflexive(apis_filtered_above_threshold))
    api_bidigraph = build_ns_graph(api_filtered)
    weakly.main(api_bidigraph, 
            f"{source_comp_unit}->{target_comp_unit}_api_filtered")
    # udfs
    udfs_carpro = make_carpro_of_dataframe(source_udfs, target_udfs)
    udf_nodewise_sim_column = udfs_carpro.apply(run_all_pairwise_feature, axis=1)
    udfs_carpro["ns_score"] = udf_nodewise_sim_column
    udfs_filtered_above_threshold = udfs_carpro[udfs_carpro.ns_score > ns_threshold]
    udf_filtered = leave_only_most_similar_pairs(
        no_reflexive(udfs_filtered_above_threshold))
    udf_bidigraph = build_ns_graph(udf_filtered)
    weakly.main(udf_bidigraph, 
            f"{source_comp_unit}->{target_comp_unit}_udf_filtered")
